...
//    }
    ...
    public ArrayList<String> getTrucksByCategory(String category){
        ArrayList<String> fbc = new ArrayList<>();

        for(int i = 0; i<foodTrucks.size(); i++){
            if(category.equals(foodTrucks.get(i).getCategory()))
                fbc.add(foodTrucks.get(i).getName());
        }
        return fbc;
    }

    public FoodTruck getTruckByName(String name){

        for(int i = 0; i<foodTrucks.size(); i++){
            if(foodTrucks.get(i).getName().equalsIgnoreCase(name))
